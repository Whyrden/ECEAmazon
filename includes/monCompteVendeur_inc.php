...
if(isset($_FILES['photo_profil']['name'])) {
	...
	$username_vendeur=$_SESSION['username_vendeur'];
...
//"images" = subdirectory for images in www directory
$target_dir = "img/imagefond/";
$target_file = $target_dir . basename($_FILES["image_fond"]["name"]);
$uploadOk = 1;
...
if(isset($_POST["button2"])) {
	$target_dir = "img/imagefond/";
	$target_file = $target_dir . basename($_FILES["image_fond"]["name"]);
 	$check = getimagesize($_FILES["image_fond"]["tmp_name"]);
 	if($check !== false) {
 		echo "Le fichier est une image - " . $check["mime"] . ".";
 		$uploadOk = 1;
 		} 
...
// Vérifier la taille du fichier
if (isset($_FILES["image_fond"]["size"]) > 500000) {
 echo "<br>" . "Désolé, votre fichier est trop volumineux.";
 $uploadOk = 0;
}
...
// si tout est correct, télécharger le fichier
...
else
 {
 	...
 	if (move_uploaded_file($_FILES["image_fond"]["tmp_name"],$target_file))
 	{
 		echo "<br>" . "Le fichier ". basename( $_FILES["image_fond"]["name"]). " a été
		téléchargé.";
 	}	
 	else 
 	{
...
if(isset($_FILES['image_fond']['name'])) {
	...
	$image_fond=$_FILES['image_fond']['name'];
	$username_vendeur=$_SESSION['username_vendeur'];

		$sql="UPDATE vendeurs SET photo_fond='$image_fond' WHERE username_vendeur='$username_vendeur'";
		...
		if(mysqli_query($db_connect,$sql)){
			header("Location: ../moncompteVendeur.php?actualisationimagefond=success");
			exit();
		}
}
